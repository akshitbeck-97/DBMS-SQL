...
use ipl ;
## ans1 
/*select bidder_id ,count(bid_status)/(select count(bid_status)  from  IPL_Bidding_Details where bid_status='won'  )from  IPL_Bidding_Details  group by bidder_id ;
select bidder_id,count(bid_status)/(select count(bid_status) from  IPL_Bidding_Details  ) from  IPL_Bidding_Details where bid_status='won'group by bidder_id ;
select  bidder_id ,count(bid_status)  from  IPL_Bidding_Details where bid_status='won' group by bidder_id ;
select bidder_id , count(bid_status)/(select count(bid_status)from  IPL_Bidding_Details where bidder_id = any(select bidder_id from  IPL_Bidding_Details where bid_status='won'group by bidder_id)  ) from  IPL_Bidding_Details where bid_status='won'group by bidder_id  ;
select bidder_id ,count(bid_status)from  IPL_Bidding_Details where bidder_id = any(select bidder_id from  IPL_Bidding_Details where bid_status='won') group by bidder_id  ;*/ 

/*select  count(bid_status)     from  IPL_Bidding_Details where bid_status='won'group by bidder_id  ;
select  bidder_id ,count(bid_status) /(select count(bid_status)  from  IPL_Bidding_Details  ) from  IPL_Bidding_Details where bid_status='won'group by bidder_id;
select count(bid_status)from  IPL_Bidding_Details where bid_status='won'group by bidder_id  ;
select bidder_id,(select count(bid_status)  from  IPL_Bidding_Details where bid_status='won')  /count(bid_status)  from  IPL_Bidding_Details group by bidder_id  ;
select (select count(bid_status)  from  IPL_Bidding_Details where bid_status='won')  /count(bid_status)  from  IPL_Bidding_Details ;
select count(bid_status)/(select count(bid_status)  from  IPL_Bidding_Details where bid_status='won' group by bidder_id )
 from  IPL_Bidding_Details  group by bidder_id ;*/
SELECT BDR_DTL.BIDDER_ID, BDR_DTL.BIDDER_NAME,
(select count(*) from ipl_bidding_details where  ipl_bidding_details.bidder_id=bdr_dtl.bidder_id AND bid_status LIKE "%won%" group by BIDDER_ID)/
(SELECT sum(no_of_bids) from ipl_bidder_points where ipl_bidder_points.BIDDER_ID=bdr_dtl.bidder_id group by bidder_id)*100 as percentage_win 
from ipl_bidder_details bdr_dtl order by percentage_win desc ;

## ans2
/* select * from IPL_Stadium;
 select * from  IPL_Match_Schedule ;*/
 ##select (match_id),(stadium_id) from IPL_Match_Schedule group by match_id ;
 select count(ims.match_id),ims.stadium_id,ips.stadium_name,ips.city from IPL_Match_Schedule ims ,ipl_stadium ips where ims.stadium_id=ips.stadium_id group by ims.stadium_id ;
select * from  IPL_Bidding_Details ;
##select stadium_name from ipl_stadium ;

## ans3
/*select count(*) /(select count(*) from ipl_stadium ips  inner join ipl_match_schedule ims on ips.stadium_id= ims.stadium_id  join ipl_match im on ims.match_id=im.match_id where toss_winner=match_winner)
from ipl_stadium ips  inner join ipl_match_schedule ims on ips.stadium_id= ims.stadium_id  join ipl_match im on ims.match_id=im.match_id ;*/
select ips.stadium_id ,ips.STADIUM_NAME,count(*) /(select count(*) from ipl_stadium ips  inner join ipl_match_schedule ims on ips.stadium_id= ims.stadium_id  join ipl_match im on ims.match_id=im.match_id )
from ipl_stadium ips  inner join ipl_match_schedule ims on ips.stadium_id= ims.stadium_id  join ipl_match im on ims.match_id=im.match_id where toss_winner=match_winner group by ips.stadium_id 
order by ips.stadium_id;
/*select ips.stadium_id,ips.STADIUM_NAME, ims.match_id,count(ims.match_id) ,im.toss_winner ,im.match_winner from ipl_stadium ips  inner join 
ipl_match_schedule ims on ips.stadium_id= ims.stadium_id  join ipl_match im on ims.match_id=im.match_id group by stadium_id; */
 select * from IPL_Stadium;
 ...
 select * from ipl_team ;
 /*select ibp.bidder_id,ibd.bid_team,ibp.no_of_bids  as total_bids 
 from IPL_Bidding_Details ibd inner join IPL_Bidder_Points ibp on ibd.bidder_id=ibp.bidder_id ;*/
 
  /*select ibp.bidder_id,ibd.bid_team,ibp.no_of_bids as total_bids ,team_name from IPL_Bidding_Details ibd inner join IPL_Bidder_Points ibp on ibd.bidder_id=ibp.bidder_id 
  left join  IPL_Team_Standings its on ibp.tournmt_id=its.tournmt_id left join IPL_Team it on its.team_id=it.team_id where ibd.bid_team=it.team_id;*/
  
    select ibd.bid_team,count(ibp.no_of_bids) as total_bids ,team_name from IPL_Bidding_Details ibd inner join IPL_Bidder_Points ibp on ibd.bidder_id=ibp.bidder_id 
  left join  IPL_Team_Standings its on ibp.tournmt_id=its.tournmt_id left join IPL_Team it on its.team_id=it.team_id where ibd.bid_team=it.team_id group by ibd.bid_team;
...
select* from IPL_Match;
select* from IPL_team;
select team_id1,win_details from ipl_match where team_id1=match_winner order by team_id1,win_details ;
 
 ## ans6
 /*select *from  IPL_Team_Standings its inner join ipl_team it on its.team_id=it.team_id  ;*/
  select TOURNMT_ID, MATCHES_PLAYED,MATCHES_LOST,MATCHES_WON ,it.team_name from  IPL_Team_Standings its inner join ipl_team it on its.team_id=it.team_id 
  group by it.team_name ,its.tournmt_id ;
 
 ##ans7
 
  select ip.player_name, itp.player_role ,it.team_name from ipl_team it  inner join ipl_team_players itp on it.team_id=itp.team_id join ipl_player ip on ip.player_id=itp.player_id 
  where team_name like'%mumbai indians%' and itp.player_role like'%bowler%';
  ## ans8
    select  count(itp.player_role) ,it.team_name from ipl_team it  inner join ipl_team_players itp on it.team_id=itp.team_id join ipl_player ip on ip.player_id=itp.player_id 
  where  itp.player_role like'all-rounder'  group by it.team_id  having count(itp.player_role)>4  order by count(itp.player_role) desc ;
